  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        setIsLogin(true);
        setError({ message: 'Account created successfully! Please log in.', type: 'success' });
      }
    } catch (error) {
        ...
        <div className="w-full max-w-md">
          ...
          {isLogin && (
            <p className="text-gray-600 mb-8">
              Don't have an account? <button onClick={() => setIsLogin(false)} className="text-fog font-medium hover:underline">Sign up</button>
            </p>
          )}
          
          {!isLogin && (
            <p className="text-gray-600 mb-8">
              Already have an account? <button onClick={() => setIsLogin(true)} className="text-fog font-medium hover:underline">Log in</button>
            </p>
          )}
          
          {error && (
            <div className={`p-3 rounded mb-4 ${error.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
              {error.message}
            </div>
          )}
            ...
            {isLogin && (
              <div className="flex items-center justify-between">
                ...
                {!resetSent ? (
                  ...
                  <span className="text-sm text-green-600">Reset link sent!</span>
                )}
              </div>
            )}
            ...
            {!isLogin && (
              <div className="flex items-center">
                ...
                <label htmlFor="terms" className="ml-2 block text-sm text-gray-700">
                  I agree to the <a href="#" className="text-fog hover:underline">Terms & Conditions</a>
                </label>
              </div>
            )}
