...
const AddProductModal = ({ showAddModal, setShowAddModal }) => {
  ...
  const validateFiles = (files) => {
    ...
    for (const f of arr) {
      if (!f.type.startsWith("image/")) return t('add_product_modal.messages.image_files_only');
    }
    return null;
  };
  ...
  const handleFiles = async (files) => {
    ...
    try {
      ...
      setThumbs(prev => [...prev, ...compressedFiles.map(file => URL.createObjectURL(file))]);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    if (missingField) return alert(t('add_product_modal.messages.fill_field', { field: missingField }));
    if (images.length === 0) return alert(t('add_product_modal.messages.upload_one_image'));
  ...
  return (
    <div className={`fixed inset-0 bg-black bg-opacity-50 z-50 ${showAddModal ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
      {/* Camera View */}
      {showCamera && (
      ...
      <div className={`fixed inset-x-0 bottom-0 md:inset-auto md:top-1/2 md:left-1/2 md:transform md:-translate-x-1/2 md:-translate-y-1/2 bg-white rounded-t-2xl md:rounded-2xl md:w-[700px] md:max-h-[90vh] transition-transform ${showAddModal ? "translate-y-0" : "translate-y-full md:translate-y-0 md:scale-95"}`}>
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-xl font-semibold">{t('add_product_modal.title')}</h2>
          ...
          <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
            {[
              ...
              <div key={name} className="relative">
                <input
                  name={name}
                  type={name === "price" || name === "in_stock" ? "number" : name === "supplier_phone" ? "tel" : "text"}
            ...
            {["category", "brand"].map((name) => (
              <div key={name} className="relative">
                ...
                >
                  <option value="" disabled>{t(`add_product_modal.placeholders.choose_${name}`)}</option>
                  {(name === "brand" ? BRANDS : ["Phone", "Accessory", "Pc", "Tablet"]).map((opt) => (
            ...
            <div className="space-y-4">
              ...
              >
                <Upload size={20} />
                <span>{isMobile ? t('add_product_modal.buttons.add_photo') : t('add_product_modal.buttons.upload_images')}</span>
              </button>
              ...
              {!isMobile && (
                ...
                >
                  ...
                  <p className="text-sm">{t('add_product_modal.messages.image_compression')}</p>
                </div>
              )}
                ...
                <div className="flex gap-2 overflow-x-auto">
                  {thumbs.map((src, i) => (
                    <div key={i} className="relative">
                      <img src={src} className="w-16 h-16 rounded object-cover" alt={`Preview ${i + 1}`} />
                      <button
        ...
        <div className="flex gap-3 p-6 border-t">
          ...
          >
            {uploading ? t('add_product_modal.buttons.processing') : t('add_product_modal.buttons.add_product')}
          </button>
        </div>
      </div>
    </div>
